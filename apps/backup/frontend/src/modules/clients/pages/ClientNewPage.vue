...
async function save() {
  ...
  const res = await apiPost<Client, Record<string, unknown>>('/clients', body)
  if (!res.ok) {
  ...
  <div class="d-flex flex-column">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
      <div>
        ...
        <h2 class="fw-bold text-dark mb-1"><i class="fas fa-user-plus me-2 text-primary no-theme"></i>Nuevo Cliente</h2>
        <p class="text-muted mb-0">Crea un cliente persona natural o empresa</p>
      </div>
      <button class="btn btn-light border rounded-pill px-4 fw-bold text-muted" type="button" @click="router.push('/clients')">Cancelar</button>
    </div>
    ...
    <div class="card shadow-sm rounded-4 border-0 overflow-hidden">
      <div class="card-body">
        ...
        <div class="d-flex justify-content-end align-items-center gap-2 flex-wrap">
          <button class="btn btn-primary rounded-pill px-5 fw-bold shadow-sm" type="button" :disabled="saving" @click="save">
            Guardar Cliente
          </button>
        </div>
      </div>
